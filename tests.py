...
def UCBTest():
    ...
    predRankCosines = [stats[t]["cosine"] for t in validTournamentNames]
    eloRankCosines  = [stats[t]["eloCosine"] for t in validTournamentNames]
    makeDoubledBarChart(validTournamentNames, predRankCosines, eloRankCosines, "Predicted Ranking", "Elo Ranking", f"Average cosine similarity to true ranking", "Tournament", "Similarity", "img/UCBTest/cosine.png", yRange=[0.99*min([min(predRankCosines), min(eloRankCosines)]),1.01])
    makeDoubledBarChart(validTournamentNames, predRankCosines, eloRankCosines, "Predicted Ranking", "Elo Ranking", f"Average cosine similarity to true ranking", "Tournament", "Similarity", "img/UCBTest/cosineWithStdBars.png", yRange=[0.99*min([min(predRankCosines), min(eloRankCosines)]),1.01], yErrors=[cosineStds, eloCosineStds])
    makeDoubledBarChart(validTournamentNames, cosineStds, eloCosineStds, "Predicted Ranking", "Elo Ranking", f"Standard deviation of similarity to true ranking", "Tournament", "Similarity", "img/UCBTest/cosineStd.png")


    # Make numMatches graphs
    numMatches = [stats[t]["numMatches"] for t in validTournamentNames]
    makeBarChart(validTournamentNames, [stats[t]["numMatches"] for t in validTournamentNames], "Average number of matches played per tournament", "Tournament", "# Matches", "img/UCBTest/numMatches.png")
    ...
    predRankCosines = [predRankCosines[i]/numMatches[i] for i in range(len(validTournamentNames))]
    eloRankCosines  = [eloRankCosines[i]/numMatches[i] for i in range(len(validTournamentNames))]
    makeDoubledBarChart(validTournamentNames, predRankCosines, eloRankCosines, "Predicted Ranking", "Elo Ranking", f"Average cosine similarity to true ranking divided by mean number of matches in tournament", "Tournament", "Similarity / # Matches", "img/UCBTest/cosineScaled.png")
